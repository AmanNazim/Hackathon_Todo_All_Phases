...
async function testRegistrationData() {
  ...
  try {
    ...
    if (userResult.rows[0].count > 0) {
      ...
      console.log('No users found. This indicates the registration process may not be working properly.');
      console.log('Checking other tables for possible data...');
    }
    ...
    // Test registration process by attempting to create auth instance
    console.log('\nTesting auth instance creation...');
    try {
      ...
      if (auth) {
        ...
        console.log('✓ Auth has database connection:', !!auth.db);
        console.log('✓ Auth email/password enabled:', auth.$emailPassword ? 'Yes' : 'No');
      } else {
        console.log('✗ Auth instance not created');
      }
    } catch (authError) {
      console.log('✗ Error creating auth instance:', authError.message);
    }
