...
user: user
command: sp.tasks
labels: [tasks,phase-1,cli,todo-app,spec-driven-development]
links:
  spec: null
  ticket: null
  adr: null
...
Your task is to generate **tasks.md** by **decomposing the existing `plan.md`** into **clear, atomic, testable, implementation-ready tasks**.

This file represents the **lowest planning layer before implementation**.
Every task MUST be traceable to `plan.md` and, indirectly, to `specification.md` (MANDATORY TO READ BOTH BEFORE EXECUTION).
...
DO NOT:
- Invent new features or subsystems
- Change ordering defined in the plan
- Write implementation code
- Merge tasks together into large blobs
- Introduce future-phase features
- Use vague tasks like “implement logic” or “handle edge cases”

DO:
- Create small, atomic tasks
- Ensure each task is independently verifiable
- Explicitly state completion & validation rules
- Maintain strict traceability to plan sections
- Write like a senior engineer preparing work for execution
...
`tasks.md` MUST:
- Serve as the **execution contract** for Claude Code
- Make implementation deterministic and reviewable
- Enable validation and quality checks per task
- Prevent ambiguity, hallucination, and scope drift

---

## 📌 INPUT MANDATORTY (MUST READ inside specs/phase-1 folder)

- `constitution.md` defines non-negotiable constraints
- `specification.md` defines system requirements
- `plan.md` defines build structure and sequencing
- This file defines **what must be built, step-by-step**
...
Explain the role of tasks.md in the spec-driven workflow.

---

### 2. Task Design Principles
Define task rules, such as:
...
Explain how tasks are grouped:
- By subsystem
- In plan-defined order
- With explicit dependencies

---

### 4. Task Groups (CORE OF THE FILE)

For **each plan subsystem**, create a task group.

Each task group MUST contain:
...
Each task MUST should like this example template:

**Task ID:** `T-XX.Y`
**Title:** Short, precise action title
**Description:** What must be built (no how)
**Inputs:** Required context or prior components
**Outputs:** Observable artifacts or behaviors
**Constraints:** Relevant rules from constitution/spec
**Validation Rules:** How correctness is verified

---

### 5. Required Task Groups (MINIMUM) (MUST USE PLAN.md FOR ALL TASKS GROUP TO INCLUDE)

Tasks MUST be generated for:
...
- Tasks that must not start early

---

### 7. Validation & Review Gates
Define:
- When tasks must be revised
- What constitutes task failure
- How hallucinated tasks are rejected

---

### 8. Out-of-Scope Confirmations
Restate:
- What tasks are intentionally excluded
...
Define:
- When tasks.md is considered complete
...
Each task MUST:
- Be implementable in one focused coding session
- Have at least one validation rule
- Be testable via CLI behavior or logs
- Avoid referencing code details or libraries
- Align exactly with the plan

---

## 📌 OUTPUT RULES

- Output **ONLY** the contents of `tasks.md`
- Markdown format
- No emojis
...
Generate a **professional, execution-ready `tasks.md`** inside specs/phase-1 folder that:
- Faithfully decomposes `plan.md`
- Enables deterministic implementation via Claude Code
- Prevents ambiguity, scope creep, and hallucination
- Demonstrates senior-level engineering discipline
...
- ✅ Impact: Created comprehensive Phase I task decomposition with 186 atomic tasks
- 🧪 Tests: none
- 📁 Files: Created specs/phase-1/tasks.md with complete task breakdown
- 🔁 Next prompts: Ready to proceed with Phase I implementation based on these tasks
- 🧠 Reflection: Tasks faithfully decompose plan into execution-ready units with proper dependencies

## Evaluation notes (flywheel)

- Failure modes observed: none
- Graders run and results (PASS/FAIL): PASS
- Prompt variant (if applicable): Phase I task generation
